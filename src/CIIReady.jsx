...
import { BETA_QUESTION_BANKS, BETA_MOCK_TIME, BETA_PASS_MARK, buildBetaMockExam } from "./betaQuestionBanks";
...
const BETA_MODULES = [
  {
    id: 'R02',
    title: 'Investment Principles and Risk',
    summary: 'Beta module planned with a compact question bank, one basic mock exam and a short conversational syllabus explainer.',
    podcastTitle: 'R02 audio explainer',
    podcastSummary: 'A short two-voice overview of the syllabus, risk concepts and the main areas that catch people out.',
    podcastEmbed: 'https://open.spotify.com/embed/episode/16HZSPT65PWtV2zZ24RoEk?utm_source=generator',
    podcastHeight: 152,
  },
  {
    id: 'R03',
    title: 'Personal Taxation',
    summary: 'Beta module planned with a lighter launch scope so new content can be published earlier without pretending it is as deep as R01.',
    podcastTitle: 'R03 audio explainer',
    podcastSummary: 'A concise run-through of allowances, reliefs, rates and the tax rules that need clean recall under pressure.',
    podcastEmbed: '',
    podcastHeight: 152,
  },
  {
    id: 'R04',
    title: 'Pensions and Retirement Planning',
    summary: 'Beta module planned with a starter bank of questions, a simple mock exam and a short overview audio episode.',
    podcastTitle: 'R04 audio explainer',
    podcastSummary: 'A practical syllabus sweep across pension types, contribution limits, retirement options and common planning traps.',
    podcastEmbed: '',
    podcastHeight: 152,
  },
  {
    ...
    summary: 'Beta module planned next because early feedback suggests clear demand for protection-focused revision.',
    podcastTitle: 'R05 audio explainer',
    podcastSummary: 'A conversational guide to life cover, income protection, critical illness and the protection advice process.',
    podcastEmbed: '',
    podcastHeight: 152,
  },
];
...
const R01_PODCAST = {
  title: 'R01 audio explainer',
  summary: 'A short conversational overview of the R01 syllabus, how the modules fit together, and the main themes worth keeping straight before you start drilling questions.',
  embed: 'https://open.spotify.com/embed/episode/4LJ8Huako76VOwPvPhsfo4?utm_source=generator',
  height: 152,
};
...
// Uses question ID as seed for consistency within a session.
function getQuestionSeed(q) {
  if (typeof q.seedId === 'number') return q.seedId;
  if (typeof q.id === 'number') return q.id;
  const text = String(q.id || '1');
  let hash = 0;
...
function shuffleOptions(q) {
  ...
  // Fisher-Yates with seeded random based on question ID
  let seed = getQuestionSeed(q) * 2654435761;
...
function saveReport(qId, qText, reportText) {
  const reports = loadReports();
  reports.push({ qId, qText: qText.substring(0, 80), report: reportText, date: new Date().toISOString() });
...
.cr-streak-fire { font-size: 20px; line-height: 1; }
.cr-streak-count { font-size: 22px; font-weight: 700; color: var(--accent); line-height: 1; }
.cr-streak-label { font-size: 12px; color: var(--text-tertiary); font-weight: 500; }
...
.cr-report-btn:hover { border-color: var(--danger); color: var(--danger); }
.cr-report-overlay {
...
.cr-report-modal h3 { font-size: 16px; margin-bottom: 4px; }
.cr-report-modal p { font-size: 13px; color: var(--text-secondary); margin-bottom: 16px; line-height: 1.5; }
.cr-report-textarea {
  width: 100%; min-height: 80px; border: 1px solid var(--border);
  border-radius: 8px; padding: 10px; font-family: var(--font);
  font-size: 13px; resize: vertical; background: var(--bg);
  color: var(--text-primary); box-sizing: border-box;
}
.cr-report-textarea:focus { outline: none; border-color: var(--accent); }
.cr-report-actions { display: flex; gap: 8px; margin-top: 12px; justify-content: flex-end; }

/* PROGRESS CHART */
.cr-chart { margin-bottom: 24px; }
.cr-chart-bars {
...
.cr-chart-bar {
  ...
  min-width: 3px; transition: height 0.3s ease;
}
.cr-chart-labels {
  display: flex; justify-content: space-between;
  margin-top: 4px;
}
.cr-chart-label { font-size: 10px; color: var(--text-tertiary); font-family: var(--mono); }
...
// ─── MAIN APP ────────────────────────────────────────────────────────────────
export default function CIIReady({ accessTier = 'full-access', modulesAccess = ['r01', 'r02', 'r03', 'r04', 'r05'] }) {
  const [view, setView] = useState('dashboard');
  const [selectedBetaModule, setSelectedBetaModule] = useState(null);
  ...
  const [betaSessionQ, setBetaSessionQ] = useState([]);
  const [betaSessionIdx, setBetaSessionIdx] = useState(0);
  const [betaChosen, setBetaChosen] = useState(null);
  const [betaPendingAnswer, setBetaPendingAnswer] = useState(null);
  const [betaSessionResults, setBetaSessionResults] = useState([]);
  const [betaMockQuestions, setBetaMockQuestions] = useState([]);
  const [betaMockIdx, setBetaMockIdx] = useState(0);
  const [betaMockAnswers, setBetaMockAnswers] = useState({});
  const [betaMockTimeLeft, setBetaMockTimeLeft] = useState(BETA_MOCK_TIME);
  const [betaMockSubmitted, setBetaMockSubmitted] = useState(false);
  const [betaMockResult, setBetaMockResult] = useState(null);
  const [betaMockReviewOpen, setBetaMockReviewOpen] = useState(false);
  const betaMockTimerRef = useRef(null);
  const allowedModules = useMemo(() => new Set((modulesAccess || []).map((moduleId) => String(moduleId).toLowerCase())), [modulesAccess]);

  // Token from localStorage (set by App.jsx)
  const token = (() => { try { return localStorage.getItem('ciiready_token'); } catch { return null; } })();

  function canAccessModule(moduleId) {
    return allowedModules.has(String(moduleId).toLowerCase());
  }

  function goToFullAccessCheckout() {
    window.location.href = 'https://r0ready.com/#buy';
  }
  ...
  useEffect(() => {
    if (view === 'beta-mock-session' && !betaMockSubmitted && betaMockTimeLeft > 0) {
      betaMockTimerRef.current = setInterval(() => {
        setBetaMockTimeLeft(t => {
          if (t <= 1) {
            clearInterval(betaMockTimerRef.current);
            return 0;
          }
          return t - 1;
        });
      }, 1000);
      return () => clearInterval(betaMockTimerRef.current);
    }
  }, [view, betaMockSubmitted, betaMockTimeLeft]);
  ...
  function submitReport() {
    if (!reportQ || !reportText.trim()) return;
    saveReport(reportQ.id, reportQ.q, reportText.trim());
    setReportSent(true);
    setTimeout(() => {
      setReportQ(null); setReportText(''); setReportSent(false);
    }, 1500);
  }
  ...
  function submitMockExam() {
    ...
    exam.questions.forEach((q, i) => {
      const sh = shuffleOptions(q);
      if (mockAnswers[i] === sh.correctIdx) correct++;
    });
  ...
  function startBetaStudy(id) {
    if (!canAccessModule(id)) return;
    const questions = BETA_QUESTION_BANKS[id] || [];
    if (questions.length === 0) return;
    setSelectedBetaModule(id);
    setBetaSessionQ(questions);
    setBetaSessionIdx(0);
    setBetaChosen(null);
    setBetaPendingAnswer(null);
    setBetaSessionResults([]);
    setView('beta-session');
  }

  function handleBetaSelectOption(idx) {
    if (betaChosen !== null) return;
    if (betaPendingAnswer === idx) {
      confirmBetaAnswer(idx);
    } else {
      setBetaPendingAnswer(idx);
    }
  }

  function confirmBetaAnswer(idx) {
    if (betaChosen !== null) return;
    const q = betaSessionQ[betaSessionIdx];
    const sh = shuffleOptions(q);
    const correct = idx === sh.correctIdx;
    setBetaChosen(idx);
    setBetaPendingAnswer(null);
    setBetaSessionResults(results => [...results, { q, chosen: idx, correct, shuffledCorrect: sh.correctIdx }]);
  }

  function nextBetaQuestion() {
    if (betaSessionIdx + 1 >= betaSessionQ.length) {
      setView('beta-summary');
    } else {
      setBetaSessionIdx(index => index + 1);
      setBetaChosen(null);
      setBetaPendingAnswer(null);
    }
  }

  function startBetaMock(id) {
    if (!canAccessModule(id)) return;
    const questions = buildBetaMockExam(id);
    if (questions.length === 0) return;
    if (betaMockTimerRef.current) clearInterval(betaMockTimerRef.current);
    setSelectedBetaModule(id);
    setBetaMockQuestions(questions);
    setBetaMockIdx(0);
    setBetaMockAnswers({});
    setBetaMockTimeLeft(BETA_MOCK_TIME);
    setBetaMockSubmitted(false);
    setBetaMockResult(null);
    setBetaMockReviewOpen(false);
    setView('beta-mock-session');
  }

  function handleBetaMockAnswer(qIdx, optIdx) {
    if (betaMockSubmitted) return;
    setBetaMockAnswers(prev => ({ ...prev, [qIdx]: optIdx }));
  }

  function submitBetaMockExam() {
    if (betaMockTimerRef.current) clearInterval(betaMockTimerRef.current);
    let correct = 0;
    betaMockQuestions.forEach((q, i) => {
      const sh = shuffleOptions(q);
      if (betaMockAnswers[i] === sh.correctIdx) correct++;
    });
    const pct = betaMockQuestions.length > 0 ? Math.round((correct / betaMockQuestions.length) * 100) : 0;
    setBetaMockSubmitted(true);
    setBetaMockResult({
      pct,
      correct,
      total: betaMockQuestions.length,
      passed: pct >= BETA_PASS_MARK,
      answers: { ...betaMockAnswers },
      timeTaken: BETA_MOCK_TIME - betaMockTimeLeft,
    });
    setView('beta-mock-result');
  }

  function openBetaModule(id) {
    if (!canAccessModule(id)) return;
    setSelectedBetaModule(id);
    setView('beta-module');
  }
  ...
  const betaModule = selectedBetaModule ? BETA_MODULES.find(mod => mod.id === selectedBetaModule) : null;
  const betaQuestionBank = betaModule ? (BETA_QUESTION_BANKS[betaModule.id] || []) : [];
  const betaTopics = betaQuestionBank.length > 0
    ? Array.from(new Set(betaQuestionBank.map(q => q.topic)))
    : [];
  const betaCurrentQ = betaSessionQ[betaSessionIdx];
  const betaMockCurrentQ = betaMockQuestions[betaMockIdx];

  // Shuffle options for current session question
  const shuffled = useMemo(() => currentQ ? shuffleOptions(currentQ) : null, [currentQ]);
  // Shuffle options for current mock question
  const mockShuffled = useMemo(() => mockCurrentQ ? shuffleOptions(mockCurrentQ) : null, [mockCurrentQ]);
  const betaShuffled = useMemo(() => betaCurrentQ ? shuffleOptions(betaCurrentQ) : null, [betaCurrentQ]);
  const betaMockShuffled = useMemo(() => betaMockCurrentQ ? shuffleOptions(betaMockCurrentQ) : null, [betaMockCurrentQ]);
    ...
    <div className="cr-app" data-theme={dark ? 'dark' : 'light'}>
      ...
      <nav className="cr-nav">
        <button className="cr-nav-logo" onClick={() => setView('dashboard')}>
          <img className="cr-nav-logo-mark" src={LOGO_URL} alt="R0Ready" />
          <span className="cr-nav-logo-text">R0Ready</span>
      ...
      {view === 'dashboard' && (
        <div className="cr-main">
          <div style={{ marginBottom: 28 }}>
            <div className="cr-section-label" style={{ marginBottom: 12 }}>R0Ready Modules</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 12 }}>
              <div style={{
                background: 'var(--bg-card)',
                border: '1px solid rgba(48,151,150,0.28)',
                borderRadius: 'var(--radius)',
                padding: '18px 18px 16px',
                boxShadow: '0 10px 22px rgba(48,151,150,0.08)',
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  <strong style={{ fontSize: 22, letterSpacing: '-0.04em' }}>R01</strong>
                  <span style={{
                    fontFamily: 'var(--mono)',
                    fontSize: 10,
                    fontWeight: 700,
                ...
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 6 }}>Financial Services, Regulation & Ethics</div>
                <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 14 }}>
                  The complete module with the full question bank, mock exams and spaced repetition engine.
                </div>
                <div style={{ fontSize: 12, color: 'var(--text-tertiary)', lineHeight: 1.6, marginBottom: 14 }}>
                  230 questions · 2 timed mocks · progress saved across devices
                </div>
                <button
                  className="cr-start-btn"
                  style={{ width: '100%' }}
                  onClick={() => {
                    if (isCaughtUp) startMockExam('A');
                    else startSession(null);
                  }}
                >
                  {isCaughtUp ? 'Take Mock A' : 'Open R01'}
                </button>
              </div>

              {BETA_MODULES.map((mod) => {
                const betaCount = (BETA_QUESTION_BANKS[mod.id] || []).length;
                const isUnlocked = canAccessModule(mod.id);
                return (
                <div key={mod.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <strong style={{ fontSize: 22, letterSpacing: '-0.04em' }}>{mod.id}</strong>
                    <span style={{
                      fontFamily: 'var(--mono)',
                      fontSize: 10,
                      fontWeight: 700,
                  ...
                  <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 6 }}>{mod.title}</div>
                  <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 14 }}>
                    {mod.summary}
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--text-tertiary)', lineHeight: 1.6, marginBottom: 14 }}>
                    {betaCount} beta questions · timed mock ready · podcast slot ready
                  </div>
                  {isUnlocked ? (
                    <button className="cr-btn-outline" style={{ width: '100%' }} onClick={() => openBetaModule(mod.id)}>
                      View beta module
                    </button>
                  ) : (
                    <div style={{
                      ...
                      <div style={{ textAlign: 'center', maxWidth: 240 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8, color: 'var(--text-tertiary)' }}>Locked</div>
                        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 6, color: 'var(--text-primary)' }}>
                          Upgrade to full access to unlock this module
                        </div>
                        <button className="cr-btn-accent" style={{ width: '100%' }} onClick={goToFullAccessCheckout}>
                          Upgrade to unlock
                        </button>
                      </div>
                    </div>
          ...
          <div style={{
            ...
            <div className="cr-section-label" style={{ marginBottom: 10 }}>R01 Audio Explainer</div>
            <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 6 }}>{R01_PODCAST.title}</div>
            <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.8, marginBottom: 16, maxWidth: 760 }}>
              {R01_PODCAST.summary}
            </div>
            <div style={{
              border: '1px solid var(--border)',
              ...
              <iframe
                title="R01 podcast"
                src={R01_PODCAST.embed}
                style={{ width: '100%', height: R01_PODCAST.height, border: 0 }}
                loading="lazy"
          ...
          {!isFirstVisit && (
            <>
              ...
              {/* Streak + mini progress chart */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', marginBottom: 20 }}>
                {streak.count > 0 && (
                  <div className="cr-streak">
                    <span className="cr-streak-fire">🔥</span>
                    <span className="cr-streak-count">{streak.count}</span>
                    <span className="cr-streak-label">day{streak.count !== 1 ? 's' : ''}</span>
                  </div>
                )}
                {totalSeen > 0 && (
                    ...
                    <div className="cr-chart">
                      <div className="cr-chart-bars">
                        {MODULES.map(mod => {
                          ...
                          return <div key={mod.num} className="cr-chart-bar" style={{ height: `${Math.max(pct, 3)}%` }} title={`M${mod.num}: ${pct}%`} />;
                        })}
                      </div>
                      <div className="cr-chart-labels">
                        <span className="cr-chart-label">M1</span>
                        <span className="cr-chart-label">M5</span>
                        <span className="cr-chart-label">M10</span>
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div className="cr-disclaimer">
            <strong>Disclaimer:</strong> R0Ready is an independent revision tool. It is not affiliated with, endorsed by, or derived from CII examination materials. All questions are independently authored and test publicly available regulatory knowledge from the FCA Handbook, UK legislation, and published regulatory guidance.
          </div>
        </div>
      )}

      {/* ═══════════════════════ SPACED REP SESSION ═══════════════════════ */}
      {view === 'beta-module' && betaModule && !canAccessModule(betaModule.id) && (
        <div className="cr-main">
          <div className="cr-summary" style={{ textAlign: 'left' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              <button className="cr-back" onClick={() => setView('dashboard')}>â†</button>
              <span className="cr-qcard-tag" style={{ marginBottom: 0 }}>{betaModule.id}</span>
              <span style={{
                fontFamily: 'var(--mono)',
                fontSize: 10,
                fontWeight: 700,
            ...
            <div className="cr-summary-score" style={{ textAlign: 'left', fontSize: 42, marginBottom: 8 }}>
              {betaModule.id}
            </div>
            <h1 className="cr-hero-title" style={{ marginBottom: 10 }}>{betaModule.title}</h1>
            <div style={{ fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.8, marginBottom: 24, maxWidth: 720 }}>
              Upgrade to full access to unlock this module.
            </div>

            <div className="cr-summary-actions" style={{ justifyContent: 'flex-start' }}>
              <button className="cr-btn-accent" onClick={goToFullAccessCheckout}>Upgrade to full access</button>
              <button className="cr-btn-outline" onClick={() => setView('dashboard')}>Back to hub</button>
              <button className="cr-btn-outline" onClick={() => startSession(null)}>Open R01 instead</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="cr-main">
          <div className="cr-summary" style={{ textAlign: 'left' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, marginBottom: 24 }}>
              {[
                { label: 'Question bank', value: `${betaQuestionBank.length} questions`, detail: 'Browse the full starter bank with instant feedback and explanations.' },
                { label: 'Mock exam', value: `${betaQuestionBank.length} questions`, detail: `Timed beta mock with a ${BETA_PASS_MARK}% pass mark and result review.` },
                { label: 'Audio explainer', value: betaModule.podcastEmbed ? 'Live now' : 'Coming soon', detail: betaModule.podcastEmbed ? 'Spotify player is embedded below for quick listening.' : 'Podcast section is ready to take a NotebookLM or ElevenLabs embed later.' },
              ].map((item) => (
                <div key={item.label} style={{
                  ...
                  <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 4 }}>{item.value}</div>
            ...
            <div style={{
              ...
              <div className="cr-section-label" style={{ marginBottom: 10 }}>Coverage snapshot</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
                {betaTopics.slice(0, 8).map((topic) => (
                  <span key={topic} className="cr-qcard-tag">{topic}</span>
                ))}
              </div>
              <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.8 }}>
                This beta module is deliberately lighter than R01. The structure is now live in-app: a starter question bank, one timed beta mock, and a reserved space for the future podcast explainer.
              </div>
            </div>
            ...
            <div style={{
              ...
              <div className="cr-section-label" style={{ marginBottom: 10 }}>Audio explainer</div>
              <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 6 }}>{betaModule.podcastTitle}</div>
              <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.8, marginBottom: 16 }}>
                {betaModule.podcastSummary}
              </div>

              {betaModule.podcastEmbed ? (
                <div style={{
                  border: '1px solid var(--border)',
                  borderRadius: 'var(--radius-sm)',
                  overflow: 'hidden',
                  ...
                  <iframe
                    title={`${betaModule.id} podcast`}
                    src={betaModule.podcastEmbed}
                    style={{ width: '100%', height: betaModule.podcastHeight || 220, border: 0 }}
                    loading="lazy"
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Coming soon</div>
                  <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.7 }}>
                    When the NotebookLM or ElevenLabs audio is ready, drop the embed URL into this module and the player will appear here without changing the layout.
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="cr-summary-actions" style={{ justifyContent: 'flex-start' }}>
              <button className="cr-btn-accent" onClick={() => startBetaStudy(betaModule.id)}>Browse questions</button>
              <button className="cr-btn-outline" onClick={() => startBetaMock(betaModule.id)}>Start beta mock</button>
              <button className="cr-btn-outline" onClick={() => setView('dashboard')}>Back to hub</button>
              <button className="cr-btn-outline" onClick={() => startSession(null)}>Open R01 instead</button>
            </div>
          </div>
      ...
      {view === 'beta-session' && betaCurrentQ && betaShuffled && (
        <div className="cr-main">
          <div className="cr-session-header">
            <button className="cr-back" onClick={() => setView('beta-module')}>←</button>
            <div className="cr-progress-bar">
              <div className="cr-progress-track">
                <div className="cr-progress-fill" style={{ width: `${(betaSessionIdx / betaSessionQ.length) * 100}%` }} />
              </div>
              <div className="cr-progress-label">{betaSessionIdx + 1} / {betaSessionQ.length}</div>
            </div>
          </div>

          <div className="cr-qcard">
            <div className="cr-qcard-meta">
              <span className="cr-qcard-tag">{betaCurrentQ.exam}</span>
              <span className="cr-qcard-tag">{betaCurrentQ.topic}</span>
              <span className={`cr-qcard-diff cr-diff-${betaCurrentQ.d}`}>
                {betaCurrentQ.d === 1 ? 'Easy' : betaCurrentQ.d === 2 ? 'Medium' : 'Hard'}
              </span>
            </div>

            <div className="cr-qcard-text">{betaCurrentQ.q}</div>

            <div className="cr-options">
              {betaShuffled.options.map((opt, i) => {
                let cls = 'cr-option';
                if (betaChosen !== null) {
                  if (i === betaShuffled.correctIdx) cls += ' cr-option--correct';
                  else if (i === betaChosen && betaChosen !== betaShuffled.correctIdx) cls += ' cr-option--wrong';
                } else if (betaPendingAnswer === i) {
                  cls += ' cr-option--pending';
                }
                return (
                  <button key={i} className={cls} onClick={() => handleBetaSelectOption(i)} disabled={betaChosen !== null}>
                    <span className="cr-option-letter">{['A', 'B', 'C', 'D'][i]}</span>
                    <span className="cr-option-text">{opt}</span>
                    {betaChosen === null && betaPendingAnswer === i && (
                      <span style={{
            ...
            {betaChosen !== null && (
              <div className="cr-explain">
                <div className={`cr-explain-result ${betaChosen === betaShuffled.correctIdx ? 'cr-explain-correct' : 'cr-explain-wrong'}`}>
                  {betaChosen === betaShuffled.correctIdx
                    ? '✓ Correct'
                    : `✕ Incorrect — answer: ${['A', 'B', 'C', 'D'][betaShuffled.correctIdx]}`
                  }
                </div>
                <div className="cr-explain-text">{betaCurrentQ.e}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                  <button className="cr-report-btn" onClick={() => { setReportQ(betaCurrentQ); setReportText(''); setReportSent(false); }}>
                    ⚑ Report an issue
                  </button>
                  <button className="cr-next" onClick={nextBetaQuestion}>
                    {betaSessionIdx + 1 < betaSessionQ.length ? 'Next →' : 'See results →'}
                  </button>
                </div>
              </div>
            )}
      ...
      {view === 'beta-summary' && betaModule && betaSessionResults.length > 0 && (
        <div className="cr-main">
          <div className="cr-summary">
            <div className="cr-summary-score" style={{ color: betaSessionResults.filter(r => r.correct).length / betaSessionResults.length >= 0.7 ? 'var(--success)' : 'var(--danger)' }}>
              {Math.round(betaSessionResults.filter(r => r.correct).length / betaSessionResults.length * 100)}%
            </div>
            <div className="cr-summary-title">{betaModule.id} question bank complete</div>
            <div className="cr-summary-meta">
              {betaSessionResults.filter(r => r.correct).length} correct · {betaSessionResults.filter(r => !r.correct).length} incorrect · {betaSessionResults.length} questions
            </div>

            <div className="cr-summary-actions">
              <button className="cr-btn-outline" onClick={() => setView('beta-module')}>Back to module</button>
              <button className="cr-btn-outline" onClick={() => startBetaMock(betaModule.id)}>Try the mock</button>
              <button className="cr-btn-accent" onClick={() => startBetaStudy(betaModule.id)}>Run again</button>
            </div>

            <div className="cr-summary-list">
              {betaSessionResults.map((r, i) => (
                <div className="cr-summary-row" key={i}>
                  ...
                  <span className="cr-summary-q">
                    {r.q.q.length > 72 ? r.q.q.substring(0, 72) + '…' : r.q.q}
                  </span>
      ...
      {view === 'beta-mock-session' && betaMockCurrentQ && betaMockShuffled && betaModule && (
        <div className="cr-main">
          <div className="cr-mock-nav">
            <div className="cr-mock-nav-left">
              <button className="cr-back" onClick={() => {
                if (confirm('Leave the beta mock? Your progress will be lost.')) {
                  if (betaMockTimerRef.current) clearInterval(betaMockTimerRef.current);
                  setView('beta-module');
                }
              }}>←</button>
              <div className="cr-progress-bar" style={{ minWidth: 120 }}>
                <div className="cr-progress-track">
                  <div className="cr-progress-fill" style={{ width: `${((betaMockIdx + 1) / betaMockQuestions.length) * 100}%` }} />
                </div>
                <div className="cr-progress-label">{betaMockIdx + 1} / {betaMockQuestions.length} · {Object.keys(betaMockAnswers).length} answered</div>
              </div>
            </div>
            <div className="cr-mock-nav-right">
              <div className={`cr-timer ${betaMockTimeLeft <= 300 ? 'cr-timer--danger' : betaMockTimeLeft <= 600 ? 'cr-timer--warning' : ''}`}>
                <span className="cr-timer-icon">⏱</span>
                {formatTime(betaMockTimeLeft)}
              </div>
              {Object.keys(betaMockAnswers).length === betaMockQuestions.length && (
                <button className="cr-mock-submit" onClick={() => {
                  if (confirm('Submit your beta mock? You cannot change answers after submission.')) submitBetaMockExam();
                }}>Submit mock</button>
              )}
            </div>
          </div>
          ...
          <div className="cr-qcard">
            <div className="cr-qcard-meta">
              <span className="cr-qcard-tag">{betaMockCurrentQ.exam || betaModule.id}</span>
              <span className="cr-qcard-tag">{betaMockCurrentQ.topic}</span>
              <span className={`cr-qcard-diff cr-diff-${betaMockCurrentQ.d}`}>
                {betaMockCurrentQ.d === 1 ? 'Easy' : betaMockCurrentQ.d === 2 ? 'Medium' : 'Hard'}
              </span>
              <span className="cr-qcard-level" style={{ color: 'var(--accent)' }}>
                Beta mock · Q{betaMockIdx + 1}
              </span>
            </div>

            <div className="cr-qcard-text">{betaMockCurrentQ.q}</div>

            <div className="cr-options">
              {betaMockShuffled.options.map((opt, i) => {
                const selected = betaMockAnswers[betaMockIdx] === i;
                return (
                  <button
                    key={i}
                    className={`cr-option ${selected ? 'cr-option--correct' : ''}`}
                    onClick={() => handleBetaMockAnswer(betaMockIdx, i)}
                  >
                    <span className="cr-option-letter">{['A', 'B', 'C', 'D'][i]}</span>
                    <span className="cr-option-text">{opt}</span>
                  </button>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 24, justifyContent: 'space-between' }}>
              <button
                ...
                onClick={() => setBetaMockIdx(i => Math.max(0, i - 1))}
                disabled={betaMockIdx === 0}
                style={{ opacity: betaMockIdx === 0 ? 0.4 : 1 }}
              >← Previous</button>
              {betaMockIdx < betaMockQuestions.length - 1 ? (
                <button className="cr-next" onClick={() => setBetaMockIdx(i => i + 1)}>
                  Next →
                </button>
              ) : (
                <button
                  ...
                  onClick={() => {
                    const unanswered = betaMockQuestions.length - Object.keys(betaMockAnswers).length;
                    const msg = unanswered > 0
                      ? `You have ${unanswered} unanswered question${unanswered !== 1 ? 's' : ''}. Submit anyway?`
                      : 'Submit your beta mock?';
                    if (confirm(msg)) submitBetaMockExam();
                  }}
                >Submit mock</button>
              )}
            </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 16, justifyContent: 'center' }}>
            {betaMockQuestions.map((_, i) => (
              <button
                key={i}
                onClick={() => setBetaMockIdx(i)}
                style={{
                  width: 28, height: 28, borderRadius: 6, border: '1px solid var(--border)',
                  background: i === betaMockIdx ? 'var(--accent)' : betaMockAnswers[i] !== undefined ? 'var(--accent-soft)' : 'var(--bg-card)',
                  color: i === betaMockIdx ? '#fff' : 'var(--text-secondary)',
      ...
      {view === 'beta-mock-result' && betaModule && betaMockResult && (
        <div className="cr-main">
          <div className="cr-summary cr-mock-result">
            <div className="cr-mock-result-header">
              <div className="cr-mock-result-pct" style={{ color: betaMockResult.passed ? 'var(--success)' : 'var(--danger)' }}>
                {betaMockResult.pct}%
              </div>
              <div className="cr-mock-result-verdict" style={{ color: betaMockResult.passed ? 'var(--success)' : 'var(--danger)' }}>
                {betaMockResult.passed ? 'Passed' : 'Not yet'} — {betaModule.id} beta mock
              </div>
              <div className="cr-mock-result-meta">
                {betaMockResult.correct}/{betaMockResult.total} correct · Pass mark: {BETA_PASS_MARK}%
              </div>
              <div className="cr-mock-result-time">
                Time: {formatTime(betaMockResult.timeTaken)} of {formatTime(BETA_MOCK_TIME)}
              </div>
            </div>

            <div className="cr-summary-actions">
              <button className="cr-btn-outline" onClick={() => setView('beta-module')}>Back to module</button>
              <button className="cr-mock-review-toggle" onClick={() => setBetaMockReviewOpen(open => !open)}>
                {betaMockReviewOpen ? 'Hide review' : 'Review answers'}
              </button>
              <button className="cr-btn-accent" onClick={() => startBetaMock(betaModule.id)}>Retake</button>
            </div>

            {betaMockReviewOpen && (
              <div>
                <div className="cr-section-label">Question review</div>
                {betaMockQuestions.map((q, i) => {
                  const sh = shuffleOptions(q);
                  const userAnswer = betaMockResult.answers[i];
                  const correct = userAnswer === sh.correctIdx;
                  return (
                    <div key={q.id} className="cr-qcard" style={{ marginBottom: 12, padding: '16px 20px' }}>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                        ...
                        <span className="cr-qcard-tag">{q.topic}</span>
                      </div>
                      <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 10, lineHeight: 1.5 }}>{q.q}</div>
                      <div className="cr-options" style={{ gap: 4 }}>
                        {sh.options.map((opt, oi) => {
                          let cls = 'cr-option';
                          if (oi === sh.correctIdx) cls += ' cr-option--correct';
                          else if (oi === userAnswer && userAnswer !== sh.correctIdx) cls += ' cr-option--wrong';
                          return (
      ...
      {view === 'session' && currentQ && (
        <div className="cr-main">
          ...
          <div className="cr-qcard">
            ...
            {chosen !== null && (
              <div className="cr-explain">
                <div className={`cr-explain-result ${chosen === shuffled.correctIdx ? 'cr-explain-correct' : 'cr-explain-wrong'}`}>
                  {chosen === shuffled.correctIdx
                    ? '✓ Correct'
                    : `✗ Incorrect — answer: ${['A','B','C','D'][shuffled.correctIdx]}`
                  }
                </div>
                <div className="cr-explain-text">{currentQ.e}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                  <button className="cr-report-btn" onClick={() => { setReportQ(currentQ); setReportText(''); setReportSent(false); }}>
                    ⚑ Report an issue
                  </button>
      ...
      {view === 'mock-session' && mockCurrentQ && (
        <div className="cr-main">
          ...
          <div className="cr-qcard">
            ...
            <div className="cr-options">
              {mockShuffled.options.map((opt, i) => {
                ...
                <>
                  ...
                  <div className="cr-summary-list" style={{ marginBottom: 24 }}>
                    {MODULES.map(mod => {
                      ...
                      const modCorrect = modQs.filter(({ q, i }) => { const sh = shuffleOptions(q); return latest.answers[i] === sh.correctIdx; }).length;
      ...
      {/* ═══════════════════════ REPORT MODAL ═══════════════════════ */}
      {reportQ && (
        <div className="cr-report-overlay" onClick={(e) => { if (e.target === e.currentTarget) { setReportQ(null); setReportText(''); } }}>
          <div className="cr-report-modal">
            {reportSent ? (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                <div style={{ fontSize: 28, marginBottom: 8 }}>✓</div>
                <div style={{ fontWeight: 600 }}>Thanks — noted!</div>
              </div>
            ) : (
              <>
                <h3>Report an issue</h3>
                <p>{typeof reportQ.id === 'number' ? `Q${reportQ.id}` : reportQ.id}: {reportQ.q.length > 60 ? reportQ.q.substring(0, 60) + '…' : reportQ.q}</p>
                <textarea
                  className="cr-report-textarea"
                  placeholder="What's wrong? e.g. 'Answer should be B not C' or 'Explanation is unclear'"
                  value={reportText}
                  onChange={e => setReportText(e.target.value)}
                  autoFocus
                />
                <div className="cr-report-actions">
                  <button className="cr-btn-outline" onClick={() => { setReportQ(null); setReportText(''); }}>Cancel</button>
                  <button className="cr-btn-accent" onClick={submitReport} disabled={!reportText.trim()}>Submit</button>
                </div>
              </>
            )}
          </div>
